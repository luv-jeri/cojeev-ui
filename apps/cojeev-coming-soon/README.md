...
The home page is the Bond: a single prompt line that six small organisms bond with, and the conversation panel that grows out of it, with the prompt lifecycle (recall, weave, reach, reply, keep, notice) played out as motion. Details are in the Bond section below. The two other story demos, The Resident (`/resident.html`) and Changing Mind (`/mind.html`), stay reachable from the switcher under the wordmark. The countdown, theme toggle, pause and the "What's coming" drawer frame the page.
