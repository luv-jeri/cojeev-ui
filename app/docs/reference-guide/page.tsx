...
import { Display, SectionTitle, Body, BodySecondary, Title } from "@/registry/sahajiv/ui/typography";
import { Badge } from "@/registry/sahajiv/ui/badge";

export const metadata = { title: "Reference effects · SahaJiv UI" };
...
export default function ReferenceGuide() {
  ...
  return <DocsPage full breadcrumb={{ enabled: false }} footer={{ enabled: false }} tableOfContent={{ enabled: false }} tableOfContentPopover={{ enabled: false }}>
    <article className="docs-article">
      <header className="docs-intro"><Badge variant="pink-soft">Reference collection</Badge><Display as="h1">Motion, made ours.</Display><BodySecondary>{rows.length} references mapped to the SahaJiv family. Existing effects keep their names; new mechanisms get their own components.</BodySecondary></header>
      ...
      <section className="docs-section"><SectionTitle as="h2">Ready for your project</SectionTitle><BodySecondary>Every component page includes a live example, copyable source, an installation command and the component API. Install the variants you need from the public SahaJiv registry.</BodySecondary></section>
    </article>
  </DocsPage>;
}
