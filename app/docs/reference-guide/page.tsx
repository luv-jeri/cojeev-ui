...
export default function ReferenceGuide() {
  ...
  return <DocsPage full breadcrumb={{ enabled: false }} footer={{ enabled: false }} tableOfContent={{ enabled: false }} tableOfContentPopover={{ enabled: false }}>
    <article className="docs-article">
      <header className="docs-intro"><Badge variant="pink-soft">Reference collection</Badge><Display as="h1">Motion, made ours.</Display><BodySecondary>{rows.length} references mapped to the Cojeev family. Existing effects keep their names; new mechanisms get their own components.</BodySecondary><DocsIntroArtwork seed="reference-guide" tone="blue" /></header>
