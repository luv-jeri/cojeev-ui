...
try {
  ...
  for(const file of await fs.readdir('out/r')) {
    ...
    const item=JSON.parse(await fs.readFile(path.join('out/r',file),'utf8'));
  ...
  const code=await new Promise((resolve,reject)=>{
    const child=spawn(process.execPath,['scripts/verify-install.mjs',`--url=${origin}`],{stdio:'inherit'});
