...
try {
  ...
  assert.equal(await page.locator(".shape-workbench-section").count(), 1, "the restored homepage includes the original shape studio");
  assert.equal(await page.locator("[data-profile-stage]").count(), 0, "the rejected profile-stage redesign is not mounted");
  await page.getByRole("heading", { level: 1, name: /Good things\s*come together\./ }).waitFor();
  mark("six live previews respond, the drawer restores focus, and the earlier homepage is restored");
