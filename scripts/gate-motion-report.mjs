...
export function writeMotionReport({rows,scenarios,fixtures,widths,modes,output='artifacts/gate-motion',report='GATE-MOTION.md'}){
 ...
 const lines=[
 '# Shared motion gate','',
 `Full manifest: **${fixtures.motionScenarios.length} scenarios × 6 viewports × 2 themes = ${full} rows**. This run selected **${total} rows** and completed **${rows.length}: ${pass} PASS, ${rows.length-pass} unresolved**. A bounded run does not close the full matrix or any of the 542 base-component isolation rows.`,
 '',
 ...
 '',
 `Selected cases: ${scenarios.map(s=>s.id).join(', ')}. Widths: ${widths.join(', ')}. Themes: ${modes.join(', ')}.`,
 '',
 '| Scenario | Width | Theme | Verdict | Source self | Candidate self | Cross fields | Nonzero pixel samples |',
 '| --- | ---: | --- | --- | --- | --- | ---: | ---: |',
 ...rows.map(r=>`| ${r.id} | ${r.width} | ${r.mode} | ${r.verdict} | ${r.oracleStable??'—'} | ${r.candidateStable??'—'} | ${r.differences?.length??'—'} | ${r.pixels?.filter(p=>p.pixels).length??'—'} |`),
 '',
