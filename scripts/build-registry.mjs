...
const items=[base,...ids.map(id=>({name:id,type:"registry:ui",title:reference[id].name,description:`${reference[id].name} with SahaJiv variants and sizing.`,registryDependencies:[`${baseURL}/r/sahajiv.json`],dependencies:[],files:[{path:`${source}/ui/${id}.tsx`,type:"registry:ui"}],css:{"@layer sahajiv-states":css(`${source}/styles/${id}.css`)},meta:{source:reference[id],api:apis[id],fidelity:"pending-spike"}}))];
base.cssVars={theme};
base.files.push(...fs.readdirSync(`${source}/motion`).filter(name=>name.endsWith(".ts")).map(name=>({path:`${source}/motion/${name}`,type:"registry:lib",target:`lib/sahajiv-motion/${name}`})));
...
for(const item of items){const file=path.join("public/r",`${item.name}.json`);const data=JSON.parse(fs.readFileSync(file,"utf8"));for(const f of data.files??[])if(f.content){f.content=f.content.replaceAll("@/registry/sahajiv/lib/utils","@/lib/utils").replaceAll("@/registry/sahajiv/motion/","@/lib/sahajiv-motion/");if(f.path===`${source}/styles/fonts.css`)f.content=f.content.replace("sahajiv-states, sahajiv-accessibility","sahajiv-states, sahajiv-morph, sahajiv-flow, sahajiv-accessibility");}fs.writeFileSync(file,JSON.stringify(data,null,2)+"\n");}
fs.copyFileSync("public/r/registry.json","public/registry.json");
console.log(`Registry built: ${items.length} items (${ids.length} candidate components; fidelity pending)`);
