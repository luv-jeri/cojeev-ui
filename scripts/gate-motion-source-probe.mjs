...
export async function probeSourceCancellation(){
 const source=fs.readFileSync('reference/cojeev-handoff-v4/js/flow.js','utf8'),scheduler=source.slice(source.indexOf('const CLK ='),source.indexOf('const isOff =')),cancellation=source.match(/timers\.forEach\(clearTimeout\)/)[0]
 ...
 return {source:'reference/cojeev-handoff-v4/js/flow.js',schedulerLines:'79–82',cancellationLine:155,cancellation,live,frozen,adapted,pass:live.length===0&&frozen.length===1&&adapted.length===0,scope:'Only frozen source object handles are canceled by replacing their callback. Native timers delegate unchanged; source files are unmodified.'}
}
