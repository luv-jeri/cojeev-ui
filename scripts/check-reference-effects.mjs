...
for (const width of (process.env.WIDTHS ?? "1440,390").split(",").map(Number)) {
  const context = await browser.newContext({ viewport: { width, height: 1050 }, deviceScaleFactor: 1, isMobile: width < 500, hasTouch: width < 500 });
  const page = await context.newPage();
  for (const entry of entries) {
    const doc = entry.existingId ?? entry.id;
    ...
    const result = { reference: entry.id, component: doc, width, checks: [], errors: [] };
    try {
      await page.emulateMedia({ reducedMotion: "no-preference" });
      const response = await page.goto(`${base}/docs/${doc}/`, { waitUntil: "networkidle" });
      assert.equal(response.status(), 200);
      await page.evaluate(() => document.fonts.ready);
      const specimen = page.locator(`[data-example="${doc}"]`).last();
      await specimen.waitFor({ state: "visible", timeout: 12000 });
      await specimen.scrollIntoViewIfNeeded();
      const variant = { "text-loop": "loop", "circular-text": "circle", "curved-loop": "arch", "text-pressure": "pressure", "clip-path": "clip" }[entry.id];
      if (variant) {
        await page.getByRole("combobox", { name: "Variant", exact: true }).click();
        await page.getByRole("option", { name: variant, exact: true }).click();
      }
      await page.waitForTimeout(450);
      await page.evaluate(mode => { document.documentElement.dataset.mode = mode; }, width < 500 ? "dark" : "light");
      const stage = specimen.locator(`[data-slot="${doc}"]`).first();
      await stage.waitFor({ state: "visible" });
      const bounds = await page.evaluate(() => ({ scroll: document.documentElement.scrollWidth, width: window.innerWidth }));
      assert(bounds.scroll <= bounds.width + 1, `Page overflows ${bounds.scroll}/${bounds.width}`);
      const box = await stage.boundingBox();
      assert(box.width > 50 && box.height > 15, `Stage has visible bounds: ${JSON.stringify(box)}`);
      result.checks.push("rendered", "viewport bounds", width < 500 ? "dark/touch" : "light/desktop");
      ...
      await page.waitForTimeout(doc === "particle-text" ? 2000 : 350);
      await specimen.screenshot({ path: `${output}/${entry.id}-${width}.png` });
      await page.emulateMedia({ reducedMotion: "reduce" }); await page.waitForTimeout(180);
      assert.notEqual(await stage.getAttribute("data-running"), "true", "Reduced motion stops optional runtime");
      const quiet = await stageSignature(stage); await page.waitForTimeout(160);
      assert.equal(await stageSignature(stage), quiet, "Reduced-motion paint stays still"); result.checks.push("dynamic reduced motion");
      assert.equal(errors.length, 0, errors.join("\n"));
      result.passed = true;
    } catch (error) { result.passed = false; result.errors.push(error.message, ...errors); result.url = page.url(); await page.screenshot({path:`${output}/${entry.id}-${width}-failure.png`}).catch(()=>{}); }
    page.off("pageerror", handler); page.off("console", consoleHandler); results.push(result);
    console.log(`${result.passed ? "PASS" : "FAIL"} ${entry.id} ${width}${result.errors.length ? " " + result.errors.join(" | ") : ""}`);
    await fs.writeFile(`${output}/results.json`, JSON.stringify(results, null, 2) + "\n");
  }
  await context.close();
}
await browser.close();
console.log(`${results.filter(row => row.passed).length}/${results.length} browser cases passed`);
