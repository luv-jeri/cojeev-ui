...
try {
  ...
  const deadline = readyStarted + 10 * 60 * 1000;
  receipt.readiness = { startedAt: new Date().toISOString(), deadlineAt: new Date(deadline).toISOString(), attempts: [] };
  const wanted = ["registry", ...ids];
  while (true) {
    const results = await Promise.all(wanted.map(async id => {
      const url = id === "registry" ? `${baseURL}/registry.json` : `${baseURL}/r/${id}.json`;
      try {
        const response = await fetch(url, { signal: AbortSignal.timeout(Math.min(10000, Math.max(1, deadline - Date.now()))) });
        const contents = await response.text();
        const data = JSON.parse(contents);
        if (!response.ok || !response.headers.get("content-type")?.includes("json") || (id === "registry" ? !Array.isArray(data.items) : data.name !== id || !Array.isArray(data.files))) throw new Error(`Invalid public registry response: HTTP${response.status}`);
        return { id, url, status: response.status, contentType: response.headers.get("content-type"), sha256: createHash("sha256").update(contents).digest("hex"), bytes: Buffer.byteLength(contents), fetchedAt: new Date().toISOString(), ...(id === "registry" ? { entryCount: data.items.length, entryNames: data.items.map(item => item.name) } : {}) };
      } catch (error) { return { id, url, error: error.message, checkedAt: new Date().toISOString() }; }
    }));
    receipt.readiness.attempts.push({ checkedAt: new Date().toISOString(), results });
    if (results.every(result => !result.error)) { receipt.publicManifest = results[0]; receipt.publicEntries = results.slice(1); break; }
    console.log(JSON.stringify({ waitingForPublicRegistry: true, failures: results.filter(result => result.error), elapsedSeconds: (Date.now() - readyStarted) / 1000 }));
    if (Date.now() >= deadline) throw new Error("Public registry was not ready within10minutes");
    await new Promise(resolve => setTimeout(resolve, Math.min(30000, deadline - Date.now())));
  }
  receipt.readiness.runtimeSeconds = (Date.now() - readyStarted) / 1000;
...
`);
  ...
  receipt.installedVersions = Object.fromEntries(Object.entries(lock.packages).filter(([name]) => name.startsWith("node_modules/")).map(([name, data]) => [name.slice("node_modules/".length), data.version]));
  receipt.directDependencies = installed.dependencies;
  receipt.directDevDependencies = installed.devDependencies;
  receipt.npmVersion = execFileSync("npm", ["--version"], { cwd: directory, encoding: "utf8", env: { ...process.env, PATH: `${path.dirname(process.execPath)}:${process.env.PATH}` } }).trim();
  receipt.shadcnVersion = execFileSync("npx", ["--yes", "shadcn@latest", "--version"], { cwd: directory, encoding: "utf8", env: { ...process.env, PATH: `${path.dirname(process.execPath)}:${process.env.PATH}` } }).trim();
  receipt.build = "PASS";
