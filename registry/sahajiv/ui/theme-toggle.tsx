...
export function ThemeToggle({mode,onModeChange,showLabel=true,className,onClick,...props}:ThemeToggleProps) {
  ...
  return <Button variant="secondary" size="sm" data-slot="theme-toggle" role="switch" aria-checked={dark} aria-label="Dark appearance" className={cn("v-theme-toggle",className)} onClick={event=>{onClick?.(event);if(!event.defaultPrevented)onModeChange(dark?"light":"dark")}} {...props}>
