...
import { createGlyphMesh, glyphGrid, normalizeGlyphOptions, type GlyphForm, type GlyphTone } from "@/registry/sahajiv/lib/glyph-mesh";
import { sculptureGeometry, type SculptureGeometryInput } from "@/registry/sahajiv/lib/sculpture-geometry";
import { rasterizeSculpture, sculptureCharacters, sculptureGrid, sculptureNumber, sculpturePrint, sculpturePrintPath, type GlyphSet, type PrintOptions } from "@/registry/sahajiv/lib/sculpture-raster";
...
export function SculptureSurface({ form = "bloom", tone = "rose", geometry, speed = 1, turn = 0, pitch = 0, zoom = 1, pointerTracking = false, paused = false, onGeometryError, className, surfaceOptions, ...props }: SculptureCommonProps & { surfaceOptions: SurfaceOptions }) {
  ...
  React.useEffect(() => {
    ...
    window.addEventListener("pointermove", pointer, { passive: true }); window.addEventListener("resize", resize); window.addEventListener("sahajiv:appearancechange", repaint); document.addEventListener("visibilitychange", visibility);
    ...
    return () => {
      ...
      resizeObserver.disconnect(); appearance.disconnect(); window.removeEventListener("pointermove", pointer); window.removeEventListener("resize", resize); window.removeEventListener("sahajiv:appearancechange", repaint); document.removeEventListener("visibilitychange", visibility);
