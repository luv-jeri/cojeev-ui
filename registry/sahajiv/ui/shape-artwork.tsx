...
function artworkModel(options: ShapeArtworkOptions) {
  ...
  if (!Object.hasOwn(signatureShapePaths, name)) throw new Error(`Unknown SahaJiv signature shape: ${name}`);
  const tone = options.tone ?? "pink";
  if (!Object.hasOwn(tones, tone)) throw new Error(`Unknown SahaJiv artwork tone: ${tone}`);
