...
No new runtime dependency is needed. Existing dirty work stays in place. Work is local and uncommitted.
...
The library imports `registry/cojeev/styles/semantic-bloom.css` alongside shared tokens. For an external app, carry the UI source, `bloom-engine.ts`, the existing motion-visibility/settings helpers and shared styles, or use the generated registry once it is published. `public/r/semantic-bloom.json` and its foundation dependency are generated locally; no public deployment was performed. The page marks this as a local addition so it does not offer an unverified public install command.
