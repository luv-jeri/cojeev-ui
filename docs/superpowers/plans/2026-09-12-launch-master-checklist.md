...
- [ ] **B02 · C — Verify the complete final-source gate.** Lint, type checking, unit tests, reporting/hosting tests, code-example checks, catalogue/browser/motion tests, reporting/analytics browser checks and clean-consumer install/build must all finish. Save sanitized evidence from the exact final revision.
  ...
  - [ ] **B02-8 · C — Name the reason a catalogue entry failed in the run log.** `scripts/check-docs.mjs` recorded `layout.error` and `preview.detail` in `results.json` but printed only their statuses, so reading why [run 34808127946](https://github.com/luv-jeri/cojeev-ui/actions/runs/34808127946) (`shape-scene` layout 1440/dark) or [run 34807702931](https://github.com/luv-jeri/cojeev-ui/actions/runs/34807702931) (`shape-scene` shared preview) failed meant opening a 256 MB artifact. A pure formatter now adds `layoutFailures` and `previewDetail` to the same per-entry line, for failures only and bounded to 400 characters, keeping every existing field and `results.json` unchanged. Both recorded messages end at the same observed stage — the click completed, then the wait for scheduled navigations timed out — which is where the run got to and not a proven cause; those failures and the `article-headings` identical-sample failure stay open and unreproduced. Focused formatter check and affected lint only; no catalogue, browser or CI run. [Scope and evidence](../../production/2026-09-14-catalogue-diagnostic-summary.md). Diagnostics visibility only: this closes no part of B02 and is not release acceptance.
