...
- [ ] **B01 · C — Repair CI runtime and scope.** Retain bounded catalogue partitions and add conservative affected-check selection for small checkpoints. Preserve exact full-release coverage, source provenance and all later checks. Scoped code acceptance permits the repair PR to merge; this runtime checkpoint closes when the combined full run finishes within its budget without hiding failures.
  ...
  - [ ] **B01-3 · C — Finish focused selection for approved component polish.** Exact choice/Accordion workbench source files receive build, generated-output consistency and affected real-browser journeys; evidence-only changes receive prose checks. Unknown/shared-engine changes and the final release retain full verification. See [cause, scope and evidence](../../production/2026-09-13-component-checkpoint-scope.md). Implementation in [PR #27](https://github.com/luv-jeri/cojeev-ui/pull/27); CI and merge remain pending.
...
- [ ] **H03 · B — Shared workbench.** Intentional responsive toolbar, working depth-background choices, discoverable motion/radius settings, state preservation, formatted/highlighted code and copy of the selected configuration; readable API tables.
  ...
  - [ ] **H03-2 · Choice-workbench polish:** implemented in [PR #25](https://github.com/luv-jeri/cojeev-ui/pull/25), source `569ab7f` (tested tree `ada276d`). Accurate glyph-picker silhouettes, visible selected state and compact comparison hierarchy; shapes, marks and selected values preserved. [Focused checks and before/after evidence](../../quality/evidence/h03-2/README.md). Rendered owner approval and applicable CI remain pending; not deployed.
