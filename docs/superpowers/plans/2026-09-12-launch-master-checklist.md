...
- Keep the owner's tracking copy of this checklist in the durable original project folder, `/Users/sanjaykumar/Documents/ChatGPT/sahajiv ui`. The release source baseline is branch `codex/000h-production-beta` at `cc971887e4e6825bd5568815cb76ebf22c602e04`; Phase 1 runs in isolated worktrees, with W01 integration in `/private/tmp/000h-phase1-jKXhib/repo`. The earlier `/private/tmp/cojeev-production-cLJFa8tY/repo` path is historical, not the current-checkout authority. Do not overwrite the durable original's dirty application source or one working copy with another.
