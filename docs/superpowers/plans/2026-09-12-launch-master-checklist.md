...
- [ ] **C10 · C — Verify GitHub synchronisation.** Valid signed webhooks update the intended report; invalid/replayed events are rejected; duplicate issue creation is prevented; explicit Released status controls completion notification.
  - [ ] **C10-1 · C — Notify the maintainer independently and expose reporting readiness.** Queue one `email_owner_received` alert per newly saved report when `REPORT_NOTIFICATION_EMAIL` is configured, delivered through the existing durable outbox to the configured maintainer only, carrying kind, reference and the authenticated inbox link but no private report text. Never backfill historical reports; keep reporter and GitHub jobs independent of an owner failure. Add `providers.ownerNotification` and `DEPLOYMENT_INTENT` to authenticated health so an intentionally staged environment stays distinguishable from an expected-active one with missing configuration, without exposing addresses. See [the focused implementation evidence](../../reporting/OPERATIONS.md). Activation stays off: `EMAIL_ENABLED=false` and a blank `DELIVERY_ACTIVATED_AT` remain committed until the coordinator verifies provider setup (C02/C12). No live email, issue or webhook provisioning is claimed by this child.
