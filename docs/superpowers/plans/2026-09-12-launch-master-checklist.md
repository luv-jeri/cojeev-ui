...
- [ ] **B01 · C — Repair CI runtime and scope.** Retain bounded catalogue partitions and add conservative affected-check selection for small checkpoints. Preserve exact full-release coverage, source provenance and all later checks. Scoped code acceptance permits the repair PR to merge; this runtime checkpoint closes when the combined full run finishes within its budget without hiding failures.
  ...
  - [ ] **B01-4 · C — Run proportionate checks for unused-code cleanup.** Add an exact-path maintenance scope for G07, with quick checks, one static build, the five landing-style consumers and browser draft persistence. Unknown changes and final releases retain the full gate. [Scope and evidence](../../production/2026-09-13-maintenance-check-scope.md). Close on reviewed checkpoint checks and merge; this does not close the parent release gate.
- [ ] **B02 · C — Verify the complete final-source gate.** Lint, type checking, unit tests, reporting/hosting tests, code-example checks, catalogue/browser/motion tests, reporting/analytics browser checks and clean-consumer install/build must all finish. Save sanitized evidence from the exact final revision.
  ...
  - [ ] **B02-3 · C — Isolate each component's browser clock.** Retire the primary behavior context between catalogue entries so simulated clocks from Agent Chat and Button cannot leak into later animation tests. Preserve product motion, assertion strength and timeouts; prove the old failure and corrected five-component sequence. [Cause and focused evidence](../../production/2026-09-13-ci-motion-completion.md). Close on reviewed checkpoint checks and merge; the combined release remains pending.
