...
| K05: shadcn submission | [Upstream PR #11899](https://github.com/shadcn-ui/ui/pull/11899) is submitted for `@000h-cojeev`; fresh installation and directory validation pass; follow-up posted on 16 September | First maintainer review and acceptance; no listing approval is claimed |
...
- The active persistent release checkout is `/Users/sanjaykumar/Developer/cojeev-ui-release`, restored from GitHub after the 13 September restart cleared the temporary worktrees. Release branch: `chore/production-beta`; restored source: `5e3570fb0762d474fbd4f6a29efd763811e3100a`. The original dirty checkout `/Users/sanjaykumar/Documents/ChatGPT/sahajiv ui` remains untouched. Earlier `/private/tmp/000h-phase1-jKXhib/` and `/private/tmp/cojeev-production-cLJFa8tY/` locations are historical, not active workspace authority. Keep further work and handoff evidence in persistent paths; use this committed checklist for current status.
...
- [x] **W01 · C — Land the handoff rules.** [PR #12](https://github.com/luv-jeri/cojeev-ui/pull/12) merged as `3730e236ac45957be29ffcf54e89de5fad1b28ff`; merge state re-read on 13 September. The checkpoint workflow and scoped AGENTS rule are in the release branch.
- [x] **W02 · C — Recover the persistent workspace and reconcile accepted checkpoints.** [PR #28](https://github.com/luv-jeri/cojeev-ui/pull/28) merged as `f3fa84babb0b0bc9dc3c132e788a7f9f39639075` after [scoped run 34741513114](https://github.com/luv-jeri/cojeev-ui/actions/runs/34741513114) passed. The persistent preview is restored and the dirty original checkout remains untouched. [Recovery record](../../production/2026-09-13-restart-recovery.md). This closes workspace recovery, not the combined release or live launch.
...
- [ ] **A01 · S — Confirm business facts.** Record the actual operator/legal name, country/state of operation, appropriate public business/contact details, intended countries served, whether paid services are sold, and intended age audience. Done when policies can be written without invented facts.
  - Confirmed by Sanjay on 13 September 2026: operator/legal name **Sanjay Kumar**, operating from **Madhya Pradesh, India**; the component library is **free**, and design/development **services are sold separately**; the public contact address stays the already-approved `hello@cojeev.com`. Published in E02-1.
  - Still unknown, and deliberately not written into any page: postal/registered address, any company registration or tax identity, intended countries served, intended age audience, service prices and refund terms. A01 stays open until these are recorded, and A05 cannot be settled without them.
  - Unverified: actual receipt at the `hello@cojeev.com` inbox. `NEXT_PUBLIC_CONTACT_ENABLED` is unchanged and still gates every published address.
...
- [ ] **A04 · B — Confirm release design.** Approve the new homepage after review, or explicitly choose the restored homepage for this release. Keep the rejected version isolated.
  - Decision recorded 13 September 2026: Sanjay explicitly chose to **keep the current/restored homepage unchanged for this launch**. No homepage file was touched by E02-1.
  - Isolation verified from the E02-1 worktree: the rejected design remains on `chore/homepage-rework-preserved` (`d6f310cb97a58bb0e96bc95329853ed8ad552097` on `origin`) and is not an ancestor of `origin/main`. The completion checkbox is left for the coordinator.
...
- [ ] **B01 · C — Repair CI runtime and scope.** Retain bounded catalogue partitions and add conservative affected-check selection for small checkpoints. Preserve exact full-release coverage, source provenance and all later checks. Scoped code acceptance permits the repair PR to merge; this runtime checkpoint closes when the combined full run finishes within its budget without hiding failures.
  - [ ] **B01-2 · C — Select focused checks for three bounded launch changes.** The conservative allowlist sends the I03 licence-notice generator, the E08-1 reporting-consent repair and the G01 standalone loading-measurement script to the full release job. Extend it by exact path only: registry generation consistency plus the real clean-consumer install for the notice generator and its generated payloads; the focused real-widget consent check plus the complete Worker-backed reporting journey and build for the reporting widget and its checks; lint, type checking and the unit suites for the measurement script, whose samples are not taken on a shared runner. Neighbouring reporting, worker, registry and script paths stay on the full job. Separate `fix(ci)` PR; no product code, dependency or release-protection changes, and a scoped pass remains checkpoint evidence, never release acceptance.
  - [x] **B01-3 · C — Finish focused selection for approved component polish.** [PR #27](https://github.com/luv-jeri/cojeev-ui/pull/27) merged as `0e6079483a142d0aba140b1c0857b8777d34f314`; [focused run 34740348050](https://github.com/luv-jeri/cojeev-ui/actions/runs/34740348050) passed. Source changes receive affected build/browser checks; evidence-only changes receive prose checks. Unknown/shared-engine changes and the final release retain full verification. This closes the scope repair only, not B01/B02 release acceptance.
  - [x] **B01-4 · C — Run proportionate checks for unused-code cleanup.** [PR #30](https://github.com/luv-jeri/cojeev-ui/pull/30) merged as `936110de17898eb33c1b44f7e6f597208753d544` after [focused run 34746852592](https://github.com/luv-jeri/cojeev-ui/actions/runs/34746852592) passed (4m42s checkpoint job). Exact-path maintenance checks cover quick tests, a static build, the five landing-style consumers and browser draft persistence. Unknown changes and final releases retain the full gate. [Scope and evidence](../../production/2026-09-13-maintenance-check-scope.md). The parent release gate remains open.
  - [ ] **B01-5 · C — Run proportionate checks for the privacy and creator copy.** The prepared E02-1 copy change edits body copy in `app/privacy/page.tsx` and `components/landing/creator-page.tsx` only; both classified as full, and the privacy page was an explicit negative case. Those two exact paths now join the existing B01-4 `maintenance` kind, which builds the site and opens every route that renders them: `/privacy/` and `/about/` through `scripts/check-landing-guides.mjs`, `/work-with-me/` through `npm run gate:marketing`, plus the server-render unit test. No new suite, workflow step, prefix or directory rule. The route files that render the component, the guide and marketing shells, the embedded analytics control, backups and every unknown path still run the full job, as do pushes, manual dispatch and any pull request into `main`. 130 classifier tests pass after 6 behavioural failures first; both selected journeys were run locally. [Scope and evidence](../../quality/2026-09-13-policy-checkpoint-scope.md). CI classification only: this closes no part of E02 and is not release acceptance.
- [ ] **B02 · C — Verify the complete final-source gate.** Lint, type checking, unit tests, reporting/hosting tests, code-example checks, catalogue/browser/motion tests, reporting/analytics browser checks and clean-consumer install/build must all finish. Save sanitized evidence from the exact final revision.
  - [ ] **B02-5 · C — Stabilise Agent Chat completion-focus observation.** Hold the demo clock until Stop has native focus, then let its real completion transition run and assert focus returns to the draft. Preserve product timing and outside focus. [Local evidence and limitations](../../production/2026-09-14-catalogue-completion-preview.md) records the delayed-driver reproduction and clock-isolation checks; required CI and merge remain pending. This does not resolve Shape Scene or certify a product-focus mutation test.
  - [ ] **B02-7 · C — Diagnose Shape Scene preview and layout failures.** Recorded CI failures occur after a completed tab/copy click while waiting for scheduled navigation. The root cause remains unconfirmed; local passes are not CI acceptance. Preserve normal-motion behavior and assertions; do not hide the failure with a blanket timeout increase.
  - [ ] **B02-6 · C — Publish under the owner's exact-source test deferral.** On 14 September Sanjay requested immediate beta/production deployment without further automated tests. Record the approved tree, omit tests honestly, retain build/artifact/private-backup/live-health and protected-production controls. [Exception](../../production/2026-09-14-owner-expedited-release.md). This does not close full B02 acceptance or claim either known failure fixed.
  - [ ] **C04-1 · C — Remove the extra R2 snapshot dependency from first deployment.** Record the existing D1 Time Travel bookmark before migration; preserve all records and require an explicit decision for any restore. [First-launch correction](../../production/2026-09-14-first-launch-recovery.md). Owner defers automated suites; no component source changes. External R2 snapshot/recovery work remains a later task.
  - [ ] **C04-2 · C — Preserve captured JSON from recovery commands.** Wrangler normal-log output carries its JSON result; suppressing that channel made a successful bookmark read unparsable. [Correction](../../production/2026-09-14-recovery-output.md). Raw subprocess output stays private; automated tests deferred by owner.
  - [ ] **B02-4 · C — Establish landing analytics fixture readiness.** Combined run34783759033 missed the Motion Drawer impression after earlier checks passed. Diagnostic-only Linux checks and local probes pass; the original timeout cause remains unconfirmed. A controlled blocked-script case demonstrates that the old fixture scrolls SSR before analytics hydrates. Wait for the real provider event before scrolling and assert visible area, preserving impression/dedup/privacy requirements and recording failure geometry. Pinned-Node positive and negative controls pass locally; close this child only after review and focused CI merge, not as blanket release acceptance. [Investigation evidence](../../production/2026-09-14-analytics-visibility.md).
  ...
  - [ ] **B02-8 · C — Name the reason a catalogue entry failed in the run log.** `scripts/check-docs.mjs` recorded `layout.error` and `preview.detail` in `results.json` but printed only their statuses, so reading why [run 34808127946](https://github.com/luv-jeri/cojeev-ui/actions/runs/34808127946) (`shape-scene` layout 1440/dark) or [run 34807702931](https://github.com/luv-jeri/cojeev-ui/actions/runs/34807702931) (`shape-scene` shared preview) failed meant opening a 256 MB artifact. A pure formatter now adds `layoutFailures` and `previewDetail` to the same per-entry line, for failures only and bounded to 400 characters, keeping every existing field and `results.json` unchanged. That bound is a length limit, not redaction, and removes no category of content. The formatter and its check are named as exact paths in both classifier maps so a printed line selects the existing bounded `transient-timing` harness instead of the full job; no wildcard, workflow or protection changes. Both recorded messages end at the same observed stage — the click completed, then the wait for scheduled navigations timed out — which is where the run got to and not a proven cause; those failures and the `article-headings` identical-sample failure stay open and unreproduced. Focused formatter check, the classifier suite and affected lint only; no catalogue, browser or CI run. [Scope and evidence](../../production/2026-09-14-catalogue-diagnostic-summary.md). Diagnostics visibility only: this closes no part of B02 and is not release acceptance.
  - [x] **B02-3 · C — Isolate each component's browser clock.** [PR #31](https://github.com/luv-jeri/cojeev-ui/pull/31) merged as `a2cd3e913ee8a9ac1ea1e8140c633fd81fb42ff7` after [focused run 34746854412](https://github.com/luv-jeri/cojeev-ui/actions/runs/34746854412) passed (11m36s checkpoint job), including positive and negative transient/clock controls. Primary also tested the combined CI changes with B01-4 before merge. Product motion, assertion strength and timeouts are unchanged. [Cause and focused evidence](../../production/2026-09-13-ci-motion-completion.md). The combined release remains pending.
...
- [ ] **C08 · C — Verify receipt-first reports.** Save and return the report receipt before attempting email/GitHub. Exercise duplicate submission, interrupted attachments, retries and provider downtime without losing or duplicating the report.
  - [ ] **C08-1 · C — Make bug-report capture selectable, visible and cancellable.** Replace This view with a rectangle Select area picker driven by pointer or keyboard, keep Full page, and show the capture phase with elapsed seconds and a Cancel that discards late results and releases the render sandbox. Keep redaction, the existing crop review before any attachment, and draft persistence. Evidence and limitations in [RF-C08-1](../../quality/rapid-fix-ledger.md#rf-c08-1--bug-report-capture-is-unselectable-and-silent-14-september). Local browser verification is separate from checkpoint merge and production acceptance.
...
- [ ] **C10 · C — Verify GitHub synchronisation.** Valid signed webhooks update the intended report; invalid/replayed events are rejected; duplicate issue creation is prevented; explicit Released status controls completion notification.
  - [ ] **C10-1 · C — Repair report notifications and delivery readiness.** Queue one private-content-free maintainer alert per new report, independently of reporter acknowledgement and GitHub delivery. Discard unrelated shared-account webhook events, preserve historical holds, and expose missing active-service configuration in authenticated health. Beta's provider setup and signing secret are configured; the source candidate enables beta with a fixed activation cutoff and the existing tester-only limits. Production stays staged pending its signing-key transfer and protected promotion. Merge, live report delivery and actual inbox receipt remain unverified. See [the repair record](../../reporting/2026-09-14-delivery-repair.md) and [operations guide](../../reporting/OPERATIONS.md).
...
- [ ] **E02 · B/L — Expand the privacy notice.** Add confirmed operator/contact details, forms/uploads/diagnostics, purposes, disclosures, public issue visibility, retention, rights/contact process and relevant transfers/children information. Verify published promises against actual behavior.
  - [ ] **E02-1 · B — Publish the confirmed operator and offer details.** The local `/privacy` source identifies the operator as Sanjay Kumar, based in Madhya Pradesh, India, and states that the library is free under MIT while services are sold separately and the site takes no payments; `/about` (and `/work-with-me`, same component) carries the same free-library/separate-services statement. See [operator-details evidence](../../quality/2026-09-13-confirmed-operator-details.md). These changes are not deployed yet. This covers A01's confirmed facts only. Forms/uploads/diagnostics disclosure, purposes, lawful bases, disclosures, public issue visibility, retention, the rights/contact process, transfers and any children/age statement are **not** covered, so E02 stays open.
  - [ ] **E02-2 · B — Explain the actual reporting data flow.** The local `/privacy` source now carries five reporting sections: required and optional report fields with the review-before-send step; local drafts in this browser, what they hold, and that `Clear draft` empties only the active kind while a browser site-data clear removes everything; optional browser details, the bounded in-memory buffer behind them, and that media is neither inspected nor blurred; private Cloudflare storage versus a receipt-first queued mirror job — thin issue, private beta repository versus public production repository — with the configured 30/180-day erasure stated as a schedule rather than a guarantee, naming the copies that outlive it — the permanent mirrored issue, the rotating pre-deployment database backups, and provider-side operational records; the Turnstile check sending IP and report reference to Cloudflare; the receipt file as a private key for status and for finishing its own declared uploads, not for report contents; and conditional queued Resend notification disclosed without any claim about current provider activation. See [reporting-notice evidence](../../quality/2026-09-13-reporting-privacy-notice.md). E08-1 corrections to the E01 inventory are recorded there. Not deployed yet, and copy only. Purposes, lawful bases, the rights/contact and erasure-request process, transfers and any children/age statement remain **not** covered, so E02 stays open.
- [ ] **E03 · B/L — Resolve cookie/storage consent.** Audit cookies, IndexedDB, local storage and tracking under the applicable jurisdictions. Add a clear cookie/storage policy; implement prior opt-in where required, easy rejection/withdrawal and persistent choices. Do not assume cookieless analytics is automatically exempt. Keep optional analytics off until its basis and behavior are verified.
  - [ ] **E03-1 · B — Hold optional analytics in future release builds.** Both GitHub environment-enable variables changed from true to false and read back on 13 September; see [configuration evidence](../../production/2026-09-13-analytics-launch-hold.md). Check exact artifact flags and final deployed network behavior before promotion; this does not disable already-deployed bytes or close E03/E04.
...
- [ ] **E08 · C — Review diagnostic timing and draft expiry.** Current diagnostics buffer before the report opens; drafts may persist locally without a TTL. Reduce collection where possible, disclose it, add appropriate expiry/clear controls and a shared-device warning; preserve useful unsent drafts deliberately.
  - [ ] **E08-1 · C — Require explicit inclusion for new bug drafts.** New bug drafts must have no attached diagnostics until the user chooses Include browser details. Verify real widget state and IndexedDB persistence, explicit inclusion, removal and reload. Preserve existing saved drafts. Background buffering and draft expiry remain separate E08 work; this child does not close them.
...
- [ ] **G07 · C — Apply the confirmed small Ponytail cleanup if approved.** Implemented and independently reviewed: remove unused `components/landing/launch-faq.tsx`, only its exclusive CSS and unused `deleteDraft` from `lib/reporting/draft.ts`. Preserve `saveDraft`, `loadDraft`, shared CSS, public APIs and safety checks. [Scoped evidence](../../quality/2026-09-13-unused-private-code.md). Close only after the maintenance-scoped PR passes and merges; no rewrite or stored-data deletion.
...
- [ ] **H03 · B — Shared workbench.** Intentional responsive toolbar, working depth-background choices, discoverable motion/radius settings, state preservation, formatted/highlighted code and copy of the selected configuration; readable API tables.
  - [x] **H03-1 · Design checkpoint:** [preservation-first polish contract](../specs/2026-09-13-preservation-first-polish-design.md) approved by Sanjay with “looks good” on 13 September; [PR #24](https://github.com/luv-jeri/cojeev-ui/pull/24), original source `67c2338`. No implementation acceptance implied. Execute the [two scoped polish checkpoints](2026-09-13-preservation-first-polish.md).
  - [x] **H03-2 · Choice-workbench polish:** [PR #25](https://github.com/luv-jeri/cojeev-ui/pull/25) merged as `55bbcb49a91734ab4323bbd7edb80b96cb60394a` after [focused run 34740914190](https://github.com/luv-jeri/cojeev-ui/actions/runs/34740914190) passed. Owner accepted the combined local preview on 13 September. Accurate glyph-picker silhouettes, visible selection and compact comparisons retain shapes, marks and answers. [Before/after evidence](../../quality/evidence/h03-2/README.md). Not deployed; broader H03 remains separate.
- [ ] **H04 · B — Shared buttons, icons and pointer stability.** Check related-component links and all sidebar/menu click targets use the intended shared controls. Reproduce movement across edges in Chrome as well as the in-app browser; confirm ghost hover, theme-icon seam, cursor flicker and overlapping panel/tab seams stay fixed.
  - [ ] **H04-1 · C — Correct dark first paint and hydration replay.** Reported on Buy Me Coffee. Shared startup repair and focused browser evidence are recorded in [RF-H04-1](../../quality/rapid-fix-ledger.md#rf-h04-1--saved-dark-appearance-flashes-light-on-page-load-13-september). Local verification is separate from checkpoint merge and production acceptance.
...
- [ ] **V50 · B — Accordion:** reopen Sanjay's reported dissatisfaction; review compact, connected and editorial approaches individually, including actual opening/closing and rapid reversal. Do not close on screenshot or count alone.
  - [x] **V50-1 · Preservation-first polish:** [PR #26](https://github.com/luv-jeri/cojeev-ui/pull/26) merged as `5e3570fb0762d474fbd4f6a29efd763811e3100a` after [focused run 34740919789](https://github.com/luv-jeri/cojeev-ui/actions/runs/34740919789) passed. Owner accepted the combined local preview on 13 September. Calmer FAQ spacing, aligned Chapters reading and subordinate Editorial artwork preserve all three approaches, keyboard focus and motion. [Before/after evidence](../../quality/evidence/v50-1/README.md). Not deployed.
...
- [x] **J02 · B — Verify Google Search Console ownership.** Use the appropriate property for the production site; add only the required verification record and preserve DNS/mail settings. Record the verified property and account owner without publishing credentials.
- [x] **J03 · B — Submit the production sitemap.** Inspect the homepage and representative docs URLs, request indexing where appropriate, and record submission dates/results. Google recommends sitemaps for many URLs and URL Inspection for individual pages; a request does not guarantee indexing. [Google guidance](https://developers.google.com/search/docs/crawling-indexing/ask-google-to-recrawl).
- [x] **J04 · B — Complete Bing discovery if approved.** Verify the production property using its current official procedure, submit the validated sitemap and record results. This is a separate account action, not an automatic consequence of Google verification.
...
- [ ] **K05 · C — Submit and handle review.** Record the upstream PR URL, date and requested changes; keep status Submitted until the maintainers actually accept it. Do not mark acceptance when a PR is merely opened.
  - [x] **K05-1 · C — Supply review evidence and one maintainer follow-up.** Depends on the existing K05 submission. Acceptance: fresh public consumer install/build, official validator on the submitted head, a short inspected demo, sanitized evidence, and one owner-authorized comment tagging the relevant maintainer. [Evidence](../../launch/2026-09-16-pr-11899-review-evidence.md) records passing checks and the posted follow-up; [checkpoint PR #70](https://github.com/luv-jeri/cojeev-ui/pull/70) merged as `a06b7be` (evidence source `368b348`). The owner subsequently requested a better demo; K05-2 tracks its replacement. K05 and K06 remain open until upstream acceptance.
  - [ ] **K05-2 · C — Replace the review demo with a designed showcase.** Depends on K05-1. Acceptance: a fresh public installation, inspected recording of real interactions, readable light/dark compositions, scoped browser/build checks, source and verification evidence, and owner visual approval before replacing the upstream clip. [Replacement evidence](../../launch/2026-09-16-pr-11899-showcase.md), source `a58a1ff`, and [draft PR #71](https://github.com/luv-jeri/cojeev-ui/pull/71). Prepared for review; approval and publication remain pending.
...
Latest verified on 13 September (these records supersede earlier pending states for the same child checkpoints): W01 merged in PR #12 (`3730e236`); B01-3 merged in PR #27 (`0e607948`) with focused run `34740348050` passing; H03-2 merged in PR #25 (`55bbcb49`) with focused run `34740914190` passing; V50-1 merged in PR #26 (`5e3570fb`) with focused run `34740919789` passing. Owner acceptance applies to the reviewed choice/Accordion preview only. These records do not close the parent library review or release gate. W02 recovery details are in [the persistent-workspace record](../../production/2026-09-13-restart-recovery.md); its own PR merge is its closing evidence.
...
| --- | --- | --- | --- | --- | --- |
| 2026-09-17 | SEO-01 / J01–J05 | SEO-01 evidence checkpoint ready; J02–J04 verified/submitted; external results pending | Source `a06b7be`; observed production `7908a61`; reviewed evidence `1746b1e` applies the original public pack; [PR #72](https://github.com/luv-jeri/cojeev-ui/pull/72) | Controller added only the presented Google TXT and Bing DNS-only CNAME. Authenticated ownership of `sc-domain:000h.cojeev.com` is verified (identity private); its one sitemap submission says Success/processed successfully, last read 9/17/26, 179 discovered pages, 0 videos. Homepage, Button, and Bento Grid inspections remain unknown/not indexed before refresh. Bing ownership is verified; its one sitemap submission is Processing. Google/Bing reports remain unavailable while processing. Analytics access is restored, but both enable flags are false and dashboard receipt is absent. Live privacy UI displayed analytics not connected after hydration; the existing 10-test analytics unit suite passed under Node v25.3.0 in 0.47 seconds. This is disabled/source evidence only. | Preserve verification records and submissions. Keep J01/J05 and E03/E04 open. Resolve the documented owner analytics choice before any exact-build enablement, deployment, or dashboard receipt gate. |
| 2026-09-16 | K05-2 | Replacement demo prepared; owner visual review pending | Evidence source `a58a1ff`; fresh public consumer | [Draft PR #71](https://github.com/luv-jeri/cojeev-ui/pull/71); [28-second showcase and scoped checks](../../launch/2026-09-16-pr-11899-showcase.md) | Approve the visual result before replacing the existing upstream clip; no second reminder |
| 2026-09-16 | K05-1 | Merged in #70 as `a06b7be`; follow-up posted | Upstream `ca9d8df`; evidence `368b348`; public registry on 16 September | [Checkpoint PR #70](https://github.com/luv-jeri/cojeev-ui/pull/70); [demo and verification record](../../launch/2026-09-16-pr-11899-review-evidence.md); six-component fresh install/build and official directory validator passed; [single follow-up](https://github.com/shadcn-ui/ui/pull/11899#issuecomment-5693390512) tags `@shadcn` | Await upstream review and Vercel preview authorization; no acceptance or full-library certification claimed |
