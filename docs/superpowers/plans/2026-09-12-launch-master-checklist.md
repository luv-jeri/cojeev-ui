...
- [ ] **B02 · C — Verify the complete final-source gate.** Lint, type checking, unit tests, reporting/hosting tests, code-example checks, catalogue/browser/motion tests, reporting/analytics browser checks and clean-consumer install/build must all finish. Save sanitized evidence from the exact final revision.
  - [ ] **B02-4 · C — Diagnose intermittent landing impression failure.** Combined run34783759033 passed catalogue/motion and downstream browser checks, then the enabled analytics fixture missed the Motion Drawer impression. The same complete fixture and CPU/slow-script probes pass locally. Capture actual component IDs and viewport/scroll/visibility state on failure in a focused Linux checkpoint run before changing product behavior or timing. Do not treat a diagnostic pass as a confirmed repair. [Investigation evidence](../../production/2026-09-14-analytics-visibility.md).
