...
**Files:** inspect scripts/build-registry.mjs, scripts/verify-install.mjs, registry.json, registry/cojeev/styles/fonts.css and licensed font sources; add opt-in local-font registry entry/assets according to shadcn's supported file-delivery format; docs/guides/local-fonts.md; docs/quality/library-integration-status.md; create focused delivery/measurement script only if existing scripts cannot express it.

- [ ] Offer an explicit self-hosted alternative to embedded font CSS. Preserve current default compatibility; ship identical WOFF2 assets plus licences and relative URL stylesheet together, with no duplicate active @font-face payload or external request. Inspect supported CLI binary asset handling before choosing packaging; if unsupported, a documented local asset install step may satisfy this opt-in route without inventing unsupported schema fields.
