...
- Analytics: access recovered through the owner's personal Chrome session to EU project 272306. Last 30 days showed 82 historical events, newest five days old, with no new deliberate Button visit/copy receipt observed. Both GitHub analytics enable flags are false under the documented launch hold. See the [baseline](baseline-2026-09-17.md) for scope and limitations; no configuration was changed.
- Existing-source check on 17 September: after hydration, the live privacy page displayed `Analytics is not connected on this site. No website events are being sent.` Separately, the existing `tests/analytics.test.ts` suite passed 10 tests with Node v25.3.0 through the release checkout's `tsx` loader in 0.47 seconds. These checks validate the current disabled/source behavior only; they do not implement or accept prior opt-in, prove the exact release artifact, or establish dashboard receipt. See [analytics readiness](analytics-readiness-2026-09-17.md).
...
1. Reuse EU project 272306 (000h by Cojeev, organization Cojeev) in the owner's personal Chrome profile; access is now verified. No replacement organisation is needed. Current beta/production enable flags are false under the [documented launch hold](../production/2026-09-13-analytics-launch-hold.md); resolve its remaining acceptance and exact-build requirements before enabling capture.
...
If report processing, inspection, or sign-in is blocked, write the exact missing user action in the baseline ledger. SEO-01's documentation-level acceptance can record that observed pending state as complete without closing the external result itself. Never put a DNS token, PostHog token, raw account identifier, visitor record, or screenshot with private content in this repository.
