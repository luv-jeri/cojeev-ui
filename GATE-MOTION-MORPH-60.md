...
- **Blob effects:** all six light rows pass. All six dark rows have zero sampled-field differences but three failing PNGs, confined to `#cat-skeleton::after`. At 360 dark the nonzero bounds are inside `[24,299,40,335]`, press `[44,298,85,337]`, release `[97,298,123,337]`. Engine-isolation source omits the complete-entry dark shimmer override at `reference/sahajiv-handoff-v4/css/components-2.css:985`, whereas production retains it in `registry/sahajiv/styles/skeleton.css:11`. This is a stylesheet-scope difference; these rows are not evidence of a new geometry bug. Their FAIL verdicts remain.
