...
- **root** `.v-nav`
- **api** .v-nav > __group + __item[aria-current=page] (pink text, 2 px marker) + __count · lives inside .v-sidebar · six destinations only — Ask SahaJiv, Needs You, Quick Note and notifications are global affordances, never nav rows
