...
- **id** `app-shell` · **tier** cojeev · **evidence** I15 · **status** source
- **root** `.v-shell`
- **api** .v-shell > .v-sidebar + .v-page (.v-topbar, .v-main > .v-content + .v-rail) · tracks are content-driven ('auto minmax(0,1fr)') with both children explicitly placed, so hiding or collapsing the rail can never squeeze the page into the rail's column · .-navbar keeps the destinations as a pill row where the rail is hidden · container-query breakpoints 1180 / 900 / 720 are written on the shell's children, because a container query never matches the element that declares the container · globals: Ask Cojeev, Needs You, Quick Note, notifications
