...
Destinations: **Work** (home) · **Automations** · **Memory** · **Library** (skills + hooks) · **AI Apps** · **Settings/System**. Global affordances, never destinations: **Ask Cojeev** (dock button / topbar pill), **Needs You** (count in the rail, read-only inbox), **Quick Note**, **notifications** (bell). Under 720 px the dock carries Work · Automations · [Ask Cojeev] · Memory · More, and More reaches Library, AI Apps, Settings plus the globals — see `fixtures/specimens.html`.
...
```
Rail 360 · gap 40 · ≤1180 rail stacks under content · ≤900 icon sidebar 76 · ≤720 sidebar hidden, `.v-dock` fixed bottom with Work · Automations · [Ask Cojeev] · Memory · More (More opens `.v-dockpanel` listing Library, AI Apps, Settings, and the globals Needs You and Quick Note).
...
## Global surfaces
- **Ask Cojeev**: `.v-assist.-mini` (pill in the topbar/rail) → `.v-assist.-anchored` (bottom-right, 470 × ≤ 760) → full sheet on mobile via `.v-drawer`. Close circle pulses once when a reply arrives.
...
## Legacy class map (ui_kits/cojeev → Cojeev Design System)
